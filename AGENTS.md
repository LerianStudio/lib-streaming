...
- Consumer surface: `streaming.NewConsumer().Brokers(...).Group(...).Apps(...).On("<resourceType>.<eventType>", fn).Build(ctx)` then `Run(ctx)`. `Apps(...)` subscribes by producing-application name (resolving to that app's one topic) AND arms source verification; `Topics(...)` is the raw escape hatch and composes with `Apps`. `On(...)` builds a `Dispatcher` that routes per event key; `Handler(...)` still takes the whole stream and is mutually exclusive with `On` (`ErrHandlerAndDispatchBothSet`). Unmatched events are IGNORED (skipped + committed) by default — `UnmatchedPolicy(UnmatchedError)` quarantines them via `ErrUnhandledEvent`. An event whose `ce-source` is not an expected producer is quarantined with `ErrUnexpectedSource` before any handler runs. The library owns commit, retry, seek-back, DLQ, tenant propagation, and rebalance safety; a consuming service implements only `streaming.Handler`. `.Enabled(false)` yields a disabled-mode no-op (the consumer kill switch). See `docs/design/consumer.md`.
...
- **ONE topic per producing application.** `Event.Topic()` returns `"lerian.streaming." + Source` and nothing else — no resource type, no event type, no schema-version suffix, ever. All traffic rides it: business facts and service-to-service commands alike. DLQ is `"lerian.streaming." + Source + ".dlq"` (`AppTopic` / `AppDLQTopic`, exported at the root). Kafka ACLs scope a producer to exactly those two names. `Event.PartitionKey()` resolves through a fallback chain: `"system:" + EventType` when `SystemEvent=true`, else `TenantID`, else `Subject` (the aggregate id), else `EventID`. The Subject/EventID rungs exist because franz-go's sticky-key partitioner branches on `key != nil` and `[]byte("")` is not nil — an empty key would hash to one constant partition and pin an entire single-tenant application stream to it. Ordering consequence: multi-tenant keeps per-tenant order, single-tenant keeps per-aggregate order via `Subject`, and an event with neither a tenant nor a subject has no ordering guarantee at all.
...
- Config-validation sentinels (also caller-correctable): `ErrMissingBrokers`, `ErrInvalidCompression`, `ErrInvalidAcks`, `ErrInvalidConfigField`, `ErrInvalidSASLMechanism`.
...
- `validateRoutesAgainstTargets` FAILS construction when any catalog definition resolves to zero `RouteRequired` routes. A definition served only by optional routes can lose every copy while `Emit` returns nil; durability must be provable at build time.
...
- `PublisherDescriptor` carries `ServiceName`, `Source` (renamed from v2's `SourceBase`; validated by `contract.ValidateSource`, not merely trimmed), `RoutePath` (defaults `/streaming`), `OutboxSupported`, `AppVersion`, `LibVersion`, and `ProducerID` (random per process, surfaced for replica disambiguation in DLQ headers and span attributes).
