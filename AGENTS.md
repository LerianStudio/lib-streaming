...
- Consumer surface: `streaming.NewConsumer().Brokers(...).Group(...).Topics(...).Handler(...).Build(ctx)` then `Run(ctx)`. The library owns commit, retry, seek-back, DLQ, tenant propagation, and rebalance safety; a consuming service implements only `streaming.Handler`. `.Enabled(false)` yields a disabled-mode no-op (the consumer kill switch). See `docs/design/consumer.md`.
...
- `Event.Topic()` derives `"{service}.<resource>.<event>"` where `service` is the sanitized `ce-source` (`sanitizeSourceSegment(Source)`) — the producing service IS the topic namespace so Kafka ACLs can scope a producer to `{service}.*` (with `".v<major>"` suffix when `SchemaVersion` major is ≥2). Sanitization is lossy: service owners must choose source values that remain unique after normalization, or distinct services can collide into the same topic and ACL namespace. This replaces the former fixed `lerian.streaming.` prefix. `Event.PartitionKey()` returns `TenantID` by default, or `"system:" + EventType` when `SystemEvent=true`.
