...
- lib-streaming provides both event production (`streaming.NewBuilder()`) and at-least-once stream consumption (`streaming.NewConsumer()`); `github.com/LerianStudio/lib-commons/v5/commons/rabbitmq` handles internal command queues. The two are orthogonal and neither deprecates the other.
