...
- Producer construction: `streaming.NewBuilder()` is the public producer entry point. The Builder validates the route table, constructs per-target transport adapters, and returns `streaming.Emitter`. Disabled-feature-flag environments use `streaming.NewNoopEmitter()` instead. `LoadConfig() (Config, []string, error)` reads every `STREAMING_*` env var, applies defaults, returns warnings, and validates the result; validation is skipped when `Enabled=false`.
