...
// unit tests that drive the per-target rt.state mirror directly. Here the
// breaker trips organically: we stop the Redpanda container, emit events,
// and wait for the real gobreaker to observe enough failures to transition
// to OPEN. The mirrored state follows via the state-change listener.
//
...
// runs → state mirrors). Any refactor that breaks the listener wiring
