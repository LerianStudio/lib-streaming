...
// caller context still completes because producer-owned close deadlines bound
// Flush and Close while caller cancellation only initiates shutdown.
func TestProducer_CloseContext_RespectsDeadline(t *testing.T) {
	...
	// CloseContext with a canceled ctx should still use producer-owned cleanup
	// contexts. The key property is that caller cancellation does not hang or
	// prevent transport cleanup.
