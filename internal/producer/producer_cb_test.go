...
func (f *fakeCB) Execute(fn func() (any, error)) (any, error) {
	...
	// observes the per-target rt.state mirror instead. This mirrors real
	// behavior where gobreaker would short-circuit AFTER trip; the T3
	// path is about the state mirror, not about what gobreaker does
	// pre-trip.
	return fn()
}
...
func TestProducer_Emit_CircuitClosed_PublishesToBroker(t *testing.T) {
	...
	if got := p.targetState("primary"); got != flagCBClosed {
		t.Errorf("initial primary target state = %d; want %d (closed)", got, flagCBClosed)
	}
	...
	if gotRec == nil {
		t.Fatalf("no record fetched from %s; kfake did not receive", "lerian.streaming.test")
	}
}
...
func TestProducer_Emit_CircuitOpen_ReturnsErrCircuitOpen(t *testing.T) {
	...
	// Force the breaker OPEN. The listener will flip the primary target's
	// rt.state mirror.
	...
	if got := p.targetState("primary"); got != flagCBOpen {
		t.Fatalf("primary target state after OPEN transition = %d; want %d", got, flagCBOpen)
	}
...
// TestProducer_CBStateListener_UpdatesTargetState verifies the listener
// observes transitions in all three directions and mirrors the state
// atomically onto the per-target rt.state.
func TestProducer_CBStateListener_UpdatesTargetState(t *testing.T) {
	cfg, _ := kfakeConfig(t)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			fakeMgr.ForceTransition(primaryService, tc.to)

			if got := p.targetState("primary"); got != tc.want {
				t.Errorf("primary target state after %s = %d; want %d", tc.name, got, tc.want)
			}
		})
	}
}
...
func TestProducer_CBStateListener_IgnoresOtherServices(t *testing.T) {
	...
	// leave the per-target state alone.
	...
	if got := p.targetState("primary"); got != flagCBClosed {
		t.Errorf("primary target state after other-service OPEN = %d; want %d (closed)", got, flagCBClosed)
	}

	// And flipping OUR service still works.
	fakeMgr.ForceTransition(p.targets["primary"].cbServiceName, circuitbreaker.StateOpen)
	if got := p.targetState("primary"); got != flagCBOpen {
		t.Errorf("primary target state after own OPEN = %d; want %d", got, flagCBOpen)
	}
}
...
//
//	The atomic.Store on rt.state MUST run BEFORE the logger.Log call
...
//	that by checking p.targetState("primary") == flagCBOpen AFTER the panic
//	has been absorbed by RecoverAndLog. A future refactor that reverses the
...
func TestProducer_CBListener_PanicSafe(t *testing.T) {
	...
	// per-target state must have been updated.
	if got := p.targetState("primary"); got != flagCBOpen {
		t.Errorf("primary target state after panicking transition = %d; want %d (atomic store runs before log call)", got, flagCBOpen)
	}
...
func TestProducer_CBListener_SubMillisecond(t *testing.T) {
	...
	for i := 0; i < samples; i++ {
		start := time.Now()
		listener.OnStateChange(context.Background(), primaryService, circuitbreaker.StateClosed, circuitbreaker.StateOpen)
...
func TestProducer_PreFlight_DoesNotFeedCB(t *testing.T) {
	...
	// Breaker must still be closed. Reading from the real manager: our
	// per-target state mirror is just that — a mirror — but since the REAL
	// breaker has not seen any Execute calls, state is still CLOSED.
	if got := p.targetState("primary"); got != flagCBClosed {
		t.Errorf("primary target state after 100 pre-flight failures = %d; want %d (closed)", got, flagCBClosed)
	}
}
...
func TestProducer_WithCircuitBreakerManager_ReusesCallerManager(t *testing.T) {
	...
	// Breaker was registered with caller's manager under our primary
	// target's service name.
	primaryService := p.targets["primary"].cbServiceName
	if got := callerMgr.GetState(primaryService); got != circuitbreaker.StateClosed {
...
func TestProducer_WithoutCallerManager_BuildsOwnManager(t *testing.T) {
	...
	// Primary target's CB service name follows the canonical pattern.
	wantPrimaryName := targetCBServiceName(p.producerID, "primary")
	rt, ok := p.targets["primary"]
	if !ok || rt == nil {
		t.Fatal("primary target not registered")
	}
	if rt.cbServiceName != wantPrimaryName {
		t.Errorf("primary cbServiceName = %q; want %q", rt.cbServiceName, wantPrimaryName)
	}

	// And the Producer pinned its primary target name.
	if p.primaryTargetName != "primary" {
		t.Errorf("primaryTargetName = %q; want %q", p.primaryTargetName, "primary")
	}
}
...
// concurrent read/write on the per-target rt.state mirror.
func TestProducer_Emit_CB_RaceWithListener(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < stateTransitions; i++ {
			...
			fakeMgr.ForceTransition(primaryService, states[i%len(states)])
		}
	}()
...
// TestProducer_BuildCBConfig_RespectsOverrides verifies that non-zero
// MultiProducerConfig values override the HTTPServiceConfig preset.
...
// TestProducer_BuildCBConfig_ZeroUsesPreset verifies that zero
// MultiProducerConfig values fall through to the HTTPServiceConfig preset.
func TestProducer_BuildCBConfig_ZeroUsesPreset(t *testing.T) {
	t.Parallel()

	mpc := MultiProducerConfig{} // all zeros
...
func TestProducer_CBStateListener_UnknownStateLogsWarning(t *testing.T) {
	...
	// Force an OPEN first so the per-target state has a known non-zero value.
	fakeMgr.ForceTransition(primaryService, circuitbreaker.StateOpen)

	if got := p.targetState("primary"); got != flagCBOpen {
		t.Fatalf("pre-unknown primary target state = %d; want %d", got, flagCBOpen)
	}

	// Drive to StateUnknown. The listener's stateToFlag returns
	// recognized=false, so the per-target state stays at its prior value.
	fakeMgr.ForceTransition(primaryService, circuitbreaker.StateUnknown)

	if got := p.targetState("primary"); got != flagCBOpen {
		t.Errorf("post-unknown primary target state = %d; want %d (unchanged)", got, flagCBOpen)
	}
}
