...
// returns true. Used to exercise the net.Error path in kafka.ClassifyError
...
func TestClassifyError_Table(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil → empty class (no error to classify)", nil, ErrorClass("")},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := kafka.ClassifyError(tt.err); got != tt.want {
				t.Errorf("kafka.ClassifyError(%v) = %q; want %q", tt.err, got, tt.want)
			}
		})
	}
}
...
// kafka.ClassifyError uses errors.Is.
func TestClassifyError_Wrapped_Unwrap(t *testing.T) {
	t.Parallel()

	wrapped := fmt.Errorf("produce attempt failed: %w", kerr.MessageTooLarge)

	if got := kafka.ClassifyError(wrapped); got != ClassSerialization {
		t.Errorf("kafka.ClassifyError(wrapped MessageTooLarge) = %q; want %q", got, ClassSerialization)
	}
}
...
// kafka.ClassifyError must unwrap once and reuse the row for the cause.
func TestClassifyError_RecordRetries_Recurses(t *testing.T) {
	...
	// kafka.ClassifyError calls errors.Is for each specific sentinel BEFORE
	...
	if got := kafka.ClassifyError(joined); got != ClassSerialization {
		t.Errorf("kafka.ClassifyError(join(ErrRecordRetries, MessageTooLarge)) = %q; want %q",
...
func TestClassifyError_RecordRetries_Opaque(t *testing.T) {
	...
	if got := kafka.ClassifyError(kgo.ErrRecordRetries); got != ClassBrokerUnavailable {
		t.Errorf("kafka.ClassifyError(ErrRecordRetries alone) = %q; want %q",
...
func TestClassifyError_ContextPrecedence(t *testing.T) {
	...
	if got := kafka.ClassifyError(wrapped); got != ClassContextCanceled {
		t.Errorf("kafka.ClassifyError(dial+Canceled) = %q; want %q", got, ClassContextCanceled)
	}
}
...
// flag. kafka.ClassifyError should route it to network_timeout.
func TestClassifyError_NetTimeout_StdLib(t *testing.T) {
	...
	if got := kafka.ClassifyError(opErr); got != ClassNetworkTimeout {
		t.Errorf("kafka.ClassifyError(net.OpError wrapping timeout) = %q; want %q",
...
func TestClassifyError_DeadlineExceededThroughTimeoutFace(t *testing.T) {
	...
	if got := kafka.ClassifyError(ctx.Err()); got != ClassContextCanceled {
		t.Errorf("kafka.ClassifyError(expired ctx err) = %q; want %q", got, ClassContextCanceled)
	}
}
