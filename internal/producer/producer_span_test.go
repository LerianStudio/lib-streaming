...
func TestEmit_Span_SingleStreamingEmitSpan(t *testing.T) {
	...
	// modes into a single key. Note: messaging.client.id and event.outcome
	// are no longer set on the top-level span (per-target identity / per-
	// route outcome live on per-route span events instead).
...
// TestEmit_Span_OutcomeAttributeReflectsBranch was deleted: it pinned the
// deleted single-target hot path's `event.outcome` top-level span attribute
// and direct ErrCircuitOpen surfacing. The multi-target runtime records
// outcomes per route via span events ("route.outbox_routed",
// "route.required_failed", "route.optional_failed") and aggregates errors
// into MultiEmitError. The replacement coverage lives in
// TestEmit_Span_SingleStreamingEmitSpan (top-level attributes),
// metrics_test.go (counter/histogram outcomes) and producer_multi_test.go
// (per-route span events).
...
// TestEmit_SpanError_RecordedOnFailure: when a required route fails after
// preflight (broker error), the span emits a "route.required_failed"
// event so trace viewers surface the failure. (The previous single-target
// path called span.RecordError to produce an "exception" event; the
// multi-target runtime uses a structured per-route event with the
...
func TestEmit_SpanError_RecordedOnFailure(t *testing.T) {
	...
	// Multi-target dispatch surfaces required-route failure as a
	// structured span event named "route.required_failed".
	foundFailureEvent := false
	for _, ev := range span.Events {
		if ev.Name == "route.required_failed" {
	...
	if !foundFailureEvent {
		t.Errorf("span missing 'route.required_failed' event; events=%v",
			spanEventNames(span))
	}
}
...
func TestEmit_Span_AttributeCountInvariant(t *testing.T) {
	...
	//  event.resource_type, event.event_type, event.definition_key,
	//  event.policy, tenant.id, streaming.producer_id, plus the multi-
	//  target counters (streaming.routes.total / streaming.routes.required).
	//
	// Note: messaging.client.id and event.outcome are no longer set on the
	// top-level span — client identity is per-target adapter (Producer-
	// level identity is streaming.producer_id) and outcome is recorded per
	// route via the deferred emit-multi recorders.
	wantKeys := []string{
		"messaging.system",
		...
		"streaming.routes.total",
		"streaming.routes.required",
	}
