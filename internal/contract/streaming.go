...
var (
	...
	// ErrInvalidRouteDefinition is returned when a RouteDefinition is
	// structurally invalid: missing key, definition key, target, invalid
	// destination, invalid route requirement, or invalid policy override.
	ErrInvalidRouteDefinition = errors.New("streaming: invalid route definition")

	// ErrInvalidDestination is returned when a Destination has an unsupported
	// transport kind, lacks both name and address, or contains header-unsafe
	// metadata.
	ErrInvalidDestination = errors.New("streaming: invalid destination")

	// ErrDuplicateRouteDefinition is returned by NewRouteTable when two route
	// definitions use the same route key. Route tables are immutable and must be
	// unambiguous.
	ErrDuplicateRouteDefinition = errors.New("streaming: duplicate route definition")

	// ErrNoRoutesConfigured is returned by NewRouteTable when called without any
	...
	// ErrMultiTransportRuntimeNotConfigured is returned by the programmatic
	// Builder when a target's transport kind has no registered adapter
	// factory. Built-in Kafka has an implicit default; other kinds require
	// an explicit factory via Builder.RegisterTransport, or the
	// transport-specific helper (Builder.SQSTarget, etc.).
	ErrMultiTransportRuntimeNotConfigured = errors.New("streaming: multi-transport runtime not configured")
	...
	// TLS 1.2 cipher suites must be on the approved AEAD/ECDHE allowlist.
	ErrInvalidTLSConfig = errors.New("streaming: invalid TLS config")

	// ErrPlaintextSASLNotAllowed is returned when SASL authentication is
	// configured without TLS. Plaintext SASL exposes reusable broker credentials
	// on the network path and is rejected by default. Use
	// WithAllowPlaintextSASL only for local/dev brokers where that risk is
	// explicitly accepted.
	...
	// outbox semantics). MongoDB transactions backed by driver v2 session-bound
	// contexts flow through context and do not use this sentinel. IsCallerError
	// returns false for it. Parallels
