...
func TestAppTopicDLQFitsKafkaLimit(t *testing.T) {
	longest := strings.Repeat("a", maxSourceSegmentBytes)
	if got := len(AppDLQTopic(longest)); got != MaxKafkaTopicNameBytes {
		t.Fatalf("len(AppDLQTopic(max source)) = %d; want %d", got, MaxKafkaTopicNameBytes)
	}
}
