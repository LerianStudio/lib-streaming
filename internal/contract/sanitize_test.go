//go:build unit

package contract

import (
	"strings"
	"testing"

	"github.com/LerianStudio/lib-observability/v4/constants"
)

// TestSanitizeBrokerURL verifies that credentials embedded in broker URLs,
// connection strings, and raw "password=" / "pass=" key/value pairs are
// stripped before any log output or error surfacing. Pattern mirrors the
// behavior of github.com/LerianStudio/lib-commons/v7/commons/rabbitmq/rabbitmq.go:129 and :470.
func TestSanitizeBrokerURL(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name           string
		in             string
		mustNotContain []string
		mustContainAny []string // at least one of these substrings must be present
	}{
		{
			name:           "sasl url with userinfo password",
			in:             "failed to connect: sasl://user:hunter2@broker.example.com:9092/cluster",
			mustNotContain: []string{"hunter2", "user:hunter2", "user@"},
			mustContainAny: []string{constants.ObfuscatedValue},
		},
		{
			name:           "sasl url with username only userinfo",
			in:             "failed to connect: sasl://access-key@broker.example.com:9092/cluster",
			mustNotContain: []string{"access-key", "access-key@"},
			mustContainAny: []string{"sasl://" + constants.ObfuscatedValue + "@broker.example.com:9092/cluster"},
		},
		{
			name:           "kafka url with userinfo password",
			in:             "dial failed kafka://admin:supersecret@kafka:9092",
			mustNotContain: []string{"supersecret", "admin:supersecret", "admin@"},
			mustContainAny: []string{constants.ObfuscatedValue},
		},
		{
			name:           "password kv pair",
			in:             "client config password=hunter2 host=broker",
			mustNotContain: []string{"hunter2"},
			mustContainAny: []string{"password=" + constants.ObfuscatedValue},
		},
		{
			name:           "pass kv pair",
			in:             "config pass=opensesame and other stuff",
			mustNotContain: []string{"opensesame"},
			mustContainAny: []string{"pass=" + constants.ObfuscatedValue},
		},
		{
			name:           "no credentials to sanitize",
			in:             "ordinary error message with no credentials",
			mustNotContain: []string{},
			mustContainAny: []string{"ordinary error message with no credentials"},
		},
		{
			// Regression guard: an "@" inside a non-URL token (e.g. an email
			// address in a log message) must NOT be treated as userinfo
			// delimiter. fallbackRedact's URL-aware splitter only fires on
			// "://" presence, so an email-only message should pass through
			// untouched.
			name:           "email address in plain text is preserved",
			in:             "user message with email@domain.com",
			mustNotContain: []string{"****", "[REDACTED]"},
			mustContainAny: []string{"email@domain.com"},
		},
		{
			name:           "secret kv pair",
			in:             "config secret=topSecret123 reason=auth_failed",
			mustNotContain: []string{"topSecret123"},
			mustContainAny: []string{"secret=" + constants.ObfuscatedValue},
		},
		{
			name:           "client_secret kv pair",
			in:             "oauth handshake failed client_secret=oauth_xxxxxx scope=read",
			mustNotContain: []string{"oauth_xxxxxx"},
			mustContainAny: []string{"client_secret=" + constants.ObfuscatedValue},
		},
		{
			name:           "client-secret kv pair (hyphen variant)",
			in:             "auth client-secret=hyphensecret scope=read",
			mustNotContain: []string{"hyphensecret"},
			mustContainAny: []string{"client-secret=" + constants.ObfuscatedValue},
		},
		{
			name:           "token kv pair",
			in:             "auth handshake failed: token=eyJhbGciOiJIUzI1NiJ9.foo.bar reason=expired",
			mustNotContain: []string{"eyJhbGciOiJIUzI1NiJ9.foo.bar"},
			mustContainAny: []string{"token=" + constants.ObfuscatedValue},
		},
		{
			name:           "bearer kv pair",
			in:             "header bearer=eyJhbGciOiJIUzI1NiJ9 path=/v1",
			mustNotContain: []string{"eyJhbGciOiJIUzI1NiJ9"},
			mustContainAny: []string{"bearer=" + constants.ObfuscatedValue},
		},
		{
			name:           "apikey kv pair",
			in:             "config apikey=ak_live_abcdef host=api.example.com",
			mustNotContain: []string{"ak_live_abcdef"},
			mustContainAny: []string{"apikey=" + constants.ObfuscatedValue},
		},
		{
			name:           "api_key kv pair (underscore variant)",
			in:             "config api_key=ak_live_xyz123 host=api.example.com",
			mustNotContain: []string{"ak_live_xyz123"},
			mustContainAny: []string{"api_key=" + constants.ObfuscatedValue},
		},
		{
			name:           "api-key kv pair (hyphen variant)",
			in:             "config api-key=ak_live_hyphen host=api.example.com",
			mustNotContain: []string{"ak_live_hyphen"},
			mustContainAny: []string{"api-key=" + constants.ObfuscatedValue},
		},
		{
			name:           "auth kv pair",
			in:             "request auth=Basic_xyz status=401",
			mustNotContain: []string{"Basic_xyz"},
			mustContainAny: []string{"auth=" + constants.ObfuscatedValue},
		},
		{
			name:           "authorization kv pair",
			in:             "header authorization=Bearer_token123 status=401",
			mustNotContain: []string{"Bearer_token123"},
			mustContainAny: []string{"authorization=" + constants.ObfuscatedValue},
		},
		{
			name:           "case insensitive: PASSWORD",
			in:             "config PASSWORD=loudCase host=broker",
			mustNotContain: []string{"loudCase"},
			mustContainAny: []string{"PASSWORD=" + constants.ObfuscatedValue},
		},
		{
			name:           "case insensitive: Token",
			in:             "header Token=mixedCase status=200",
			mustNotContain: []string{"mixedCase"},
			mustContainAny: []string{"Token=" + constants.ObfuscatedValue},
		},
		{
			name:           "authorization bearer header",
			in:             "request failed Authorization: Bearer eyJhbGciOiJIUzI1NiJ9 status=401",
			mustNotContain: []string{"eyJhbGciOiJIUzI1NiJ9"},
			mustContainAny: []string{"Authorization: Bearer " + constants.ObfuscatedValue},
		},
		{
			name:           "authorization basic header",
			in:             "request failed Authorization: Basic dXNlcjpwYXNz status=401",
			mustNotContain: []string{"dXNlcjpwYXNz"},
			mustContainAny: []string{"Authorization: Basic " + constants.ObfuscatedValue},
		},
		{
			name:           "authorization basic key value redacts whole value",
			in:             "request failed Authorization=Basic dXNlcjpwYXNz status=401",
			mustNotContain: []string{"Basic dXNlcjpwYXNz", "dXNlcjpwYXNz"},
			mustContainAny: []string{"Authorization=" + constants.ObfuscatedValue},
		},
		{
			name:           "auth token header",
			in:             "request failed Auth: Token abc status=401",
			mustNotContain: []string{"abc"},
			mustContainAny: []string{"Auth: Token " + constants.ObfuscatedValue},
		},
		{
			name:           "authorization arbitrary scheme header",
			in:             "request failed Authorization: CustomScheme secret-value status=401",
			mustNotContain: []string{"secret-value"},
			mustContainAny: []string{"Authorization: CustomScheme " + constants.ObfuscatedValue},
		},
		{
			name:           "auth arbitrary scheme key value redacts whole value",
			in:             "request failed auth=Token abc status=401",
			mustNotContain: []string{"Token abc", "abc"},
			mustContainAny: []string{"auth=" + constants.ObfuscatedValue},
		},
		{
			name:           "AWS signed URL query credentials",
			in:             "https://sqs.us-east-1.amazonaws.com/123/queue?X-Amz-Credential=AKIAIOSFODNN7EXAMPLE%2F20260504&X-Amz-Signature=abcdef",
			mustNotContain: []string{"AKIAIOSFODNN7EXAMPLE", "abcdef"},
			mustContainAny: []string{"X-Amz-Credential=" + constants.ObfuscatedValue, "X-Amz-Signature=" + constants.ObfuscatedValue},
		},
		{
			name:           "AWS session token query credential",
			in:             "https://sqs.us-east-1.amazonaws.com/123/queue?X-Amz-Security-Token=sessiontokenvalue",
			mustNotContain: []string{"sessiontokenvalue"},
			mustContainAny: []string{"X-Amz-Security-Token=" + constants.ObfuscatedValue},
		},
		{
			name:           "AWS snake session token kv",
			in:             "config aws_session_token=sessiontokenvalue region=us-east-1",
			mustNotContain: []string{"sessiontokenvalue"},
			mustContainAny: []string{"aws_session_token=" + constants.ObfuscatedValue},
		},
		{
			name:           "bare AWS access key ID",
			in:             "auth failed for access key AKIAIOSFODNN7EXAMPLE",
			mustNotContain: []string{"AKIAIOSFODNN7EXAMPLE"},
			mustContainAny: []string{constants.ObfuscatedValue},
		},
		{
			name:           "bare AWS session access key ID",
			in:             "auth failed for access key ASIAIOSFODNN7EXAMPLE",
			mustNotContain: []string{"ASIAIOSFODNN7EXAMPLE"},
			mustContainAny: []string{constants.ObfuscatedValue},
		},
		{
			name:           "PII email kv pair from lib-observability defaults",
			in:             "profile email=customer@example.com source=broker",
			mustNotContain: []string{"customer@example.com"},
			mustContainAny: []string{"email=" + constants.ObfuscatedValue},
		},
		{
			name:           "financial account number kv pair from lib-observability defaults",
			in:             "routing account_number=1234567890 source=broker",
			mustNotContain: []string{"1234567890"},
			mustContainAny: []string{"account_number=" + constants.ObfuscatedValue},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()

			out := sanitizeBrokerURL(tt.in)

			for _, s := range tt.mustNotContain {
				if strings.Contains(out, s) {
					t.Errorf("sanitizeBrokerURL(%q) = %q; must NOT contain %q", tt.in, out, s)
				}
			}

			if len(tt.mustContainAny) > 0 {
				found := false
				for _, s := range tt.mustContainAny {
					if strings.Contains(out, s) {
						found = true
						break
					}
				}
				if !found {
					t.Errorf("sanitizeBrokerURL(%q) = %q; expected at least one of %v", tt.in, out, tt.mustContainAny)
				}
			}
		})
	}
}

// TestSanitizeBrokerURL_EmptyInput asserts the empty-string fast-path
// explicitly. Table-driven mustContainAny:[]string{""} is vacuously true
// (strings.Contains returns true for the empty substring), so this case
// needs a direct equality check to pin the contract.
func TestSanitizeBrokerURL_EmptyInput(t *testing.T) {
	t.Parallel()

	out := sanitizeBrokerURL("")
	if out != "" {
		t.Errorf("sanitizeBrokerURL(%q) = %q; want %q", "", out, "")
	}
}
