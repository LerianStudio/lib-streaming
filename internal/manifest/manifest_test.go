...
func TestManifest_InvalidDescriptor(t *testing.T) {
	...
	_, err := BuildManifest(PublisherDescriptor{}, Catalog{}, RouteTable{})
...
func TestManifest_DeterministicJSON(t *testing.T) {
	...
	manifest1, err := BuildManifest(descriptor, catalog1, RouteTable{})
	if err != nil {
		t.Fatalf("BuildManifest(1) error = %v", err)
	}
	manifest2, err := BuildManifest(descriptor, catalog2, RouteTable{})
	if err != nil {
		t.Fatalf("BuildManifest(2) error = %v", err)
	}
	manifest3, err := BuildManifest(descriptor, catalog3, RouteTable{})
	if err != nil {
