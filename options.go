...
// WithSASL sets the SASL mechanism for broker authentication. SASL requires
// TLS by default; constructing with SASL and no TLS returns
// ErrPlaintextSASLNotAllowed unless WithAllowPlaintextSASL is also supplied.
func WithSASL(mechanism sasl.Mechanism) EmitterOption { return producer.WithSASL(mechanism) }

// WithAllowPlaintextSASL permits SASL without TLS for local/dev brokers only.
