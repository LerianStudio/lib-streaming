...
func TestIsCallerError(t *testing.T) {
	...
	callerCases := []struct {
		...
		{"ErrEmptyPayload", streaming.ErrEmptyPayload},
		...
		{"ErrInvalidRouteDefinition", streaming.ErrInvalidRouteDefinition},
		...
		{"ErrPlaintextSASLNotAllowed", streaming.ErrPlaintextSASLNotAllowed},
	}
	...
	infraCases := []struct {
		...
		{"ClassTopicNotFound", &streaming.EmitError{Class: streaming.ClassTopicNotFound}},
		{"ClassBrokerUnavailable", &streaming.EmitError{Class: streaming.ClassBrokerUnavailable}},
		{"ClassNetworkTimeout", &streaming.EmitError{Class: streaming.ClassNetworkTimeout}},
		{"ClassContextCanceled", &streaming.EmitError{Class: streaming.ClassContextCanceled}},
		{"ClassBrokerOverloaded", &streaming.EmitError{Class: streaming.ClassBrokerOverloaded}},
		{"ErrEmitterClosed", streaming.ErrEmitterClosed},
		{"ErrCircuitOpen", streaming.ErrCircuitOpen},
		{"ErrOutboxNotConfigured", streaming.ErrOutboxNotConfigured},
		{"ErrOutboxTxUnsupported", streaming.ErrOutboxTxUnsupported},
		{"ErrNilProducer", streaming.ErrNilProducer},
		{"ErrNilOutboxRegistry", streaming.ErrNilOutboxRegistry},
	...
	for _, tc := range callerCases {
		t.Run("caller/"+tc.name, func(t *testing.T) {
			t.Parallel()
			if !streaming.IsCallerError(tc.err) {
...
func TestEmitError_ErrorRedactsCredentials(t *testing.T) {
	...
	ee := &streaming.EmitError{
		ResourceType: "transaction",
		EventType:    "created",
		TenantID:     "t-123",
		Topic:        "lerian.streaming.transaction.created",
		Class:        streaming.ClassBrokerUnavailable,
		Cause:        cause,
	}
	...
	if !strings.Contains(got, string(streaming.ClassBrokerUnavailable)) {
		t.Errorf("EmitError.Error() = %q; expected class token %q", got, streaming.ClassBrokerUnavailable)
	}
}
...
func TestEmitError_Unwrap(t *testing.T) {
	...
	ee := &streaming.EmitError{
		Class: streaming.ClassNetworkTimeout,
		Cause: sentinel,
	}
...
func TestEmitError_ErrorNoCause(t *testing.T) {
	...
	ee := &streaming.EmitError{
		...
		EventType:    "created",
		Topic:        "lerian.streaming.transaction.created",
		Class:        streaming.ClassValidation,
		Cause:        nil,
	}

	got := ee.Error()
	if !strings.Contains(got, string(streaming.ClassValidation)) {
		t.Errorf("Error() = %q; want to contain %q", got, streaming.ClassValidation)
	}
}
...
func TestEmitError_AsTarget(t *testing.T) {
	...
	ee := &streaming.EmitError{
		...
		Class:        streaming.ClassSerialization,
	...
	var target *streaming.EmitError
	...
	if target.Class != streaming.ClassSerialization {
		t.Errorf("target.Class = %q; want %q", target.Class, streaming.ClassSerialization)
	}
}
...
func TestSentinelErrors_IsMatches(t *testing.T) {
	...
	sentinels := []error{
		...
		streaming.ErrDuplicateEventDefinition,
