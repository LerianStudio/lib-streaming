...
- DLQ headers must include exactly the documented `x-lerian-dlq-*` fields. Nine are on every entry: source topic, error class, error message, retry count, first failure timestamp, producer ID, source partition, source offset, and cause kind. Two more are payload markers, present only when the payload was dropped to fit the broker limit: payload-omitted and payload-bytes.
- The nine are hop-scoped: they describe one quarantine, so a writer REPLACES them rather than appending, and the route back is a chain walked one hop at a time. The two payload markers are record-scoped and are carried forward across hops; a hop that drops the payload restates its own measurement of what it dropped.
