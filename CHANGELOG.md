...
- **`Build` now refuses a consumer subscribed to its own quarantine topic** (`ErrSubscribedToOwnQuarantineTopic`). `Source(...)` names where a consumer quarantines, so a consumer that also subscribes there republishes onto the topic it just read from and is redelivered, quarantined, redelivered — forever, reporting healthy and growing the topic without bound. Both strings are known at construction. A DLQ reader drains another application's `.dlq` under its own `ce-source`: `Source("lender-dlq-desk")`, `Topics("lerian.streaming.lender.dlq")`. This can reject an existing consumer, but only one already in that loop.
