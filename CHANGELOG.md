...
- `DiscardHandler` is mutually exclusive with `Handler`, `On`/`OnFrom` and `Commands` (`ErrDiscardHandlerAndHandlerBothSet`), enforced in either order. Previously the two whole-stream modes wrote the same field and the last call silently won — demoting a DLQ reader to a plain handler while it was still subscribed to a `.dlq` topic.
