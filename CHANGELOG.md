...
Features:
- **An event whose `datacontenttype` is NOT JSON can now be persisted through the OUTBOX route.** Direct emit already carried opaque bytes — a definition declaring `application/xml`, `text/xml; charset=ISO-8859-1` or `application/octet-stream` skips the `json.Valid` gate and the payload ships verbatim as the Kafka record value — but the same event could not be written to the outbox at all. `Event.Payload` is `json.RawMessage`, whose `MarshalJSON` hands its bytes to the encoder and has them VALIDATED on the way out, so `json.Marshal` of the outbox envelope failed with `invalid character '<' looking for beginning of value` before any broker was involved. A service handing an SFN XML document to the transactional outbox route got a persist-time error, while the identical document emitted directly succeeded — which is why the field's doc comment ("sent unchanged as the Kafka value") read as true. `Event` now carries a symmetric `MarshalJSON`/`UnmarshalJSON` pair that puts a non-JSON payload in an explicit `PayloadOpaque` field (base64, the only lossless way to carry arbitrary bytes inside the JSONB column lib-commons stores the row in — a plain JSON string must be valid UTF-8 and Go's encoder replaces invalid UTF-8 with U+FFFD, returning no error, so an ISO-8859-1 document would come back silently corrupted) and restores those bytes on read, so the relay republishes the original. **No new option, constructor or flag: declaring the content type on the `EventDefinition` is the whole API**, and it is the same declaration the direct path already honoured.
- **Nothing about the JSON path changes.** A JSON payload is still persisted inline under `"Payload"` with no `"PayloadOpaque"` key, so rows written by this version are byte-identical to rows written by every previous one and the operator SQL in `MIGRATION-v4.md` keeps working. Malformed bytes declared as `application/json` still fail loudly at marshal rather than being quietly rerouted through the opaque field — the discriminator is the DECLARED content type, never a `json.Valid` probe of the bytes, which is also what keeps the JSON hot path free of a second full scan. On read the opaque field is honoured whenever it is PRESENT rather than re-derived from the content type, so an operator editing `DataContentType` on a persisted row cannot thereby erase the payload.
- **`ErrEmptyPayload`: an event with no body is now refused by name, for every content type, before anything is persisted or published.** It is the lower end of the same length check `ErrPayloadTooLarge` guards, and it replaces three different bad outcomes. An empty JSON payload used to return `ErrNotJSON` — "payload must be valid JSON" is a misleading diagnosis for a body the caller simply forgot to set. An empty OPAQUE payload passed every gate and then died inside the outbox envelope marshal with `unexpected end of JSON input`, a message naming neither the field nor the caller. And a NIL opaque payload was the silent one: it passed everything, persisted as the JSON literal `null`, and decoded back as the four bytes `null`, so the relay would have published the word `null` as the record value of a regulatory document with no error raised anywhere. The check is on LENGTH, not on content type, so nil and zero-length are both refused, at the caller gate (`NewEmitRequest`) and again at producer preflight — the second copy guards the other input, an outbox row decoded on replay, where the bytes may have been edited or truncated at rest. `ErrEmptyPayload` is caller-correctable (`IsCallerError` returns true), so a relay that classifies on it invalidates such a row rather than retrying it forever. **This changes the error a caller sees for an empty JSON payload** (`ErrNotJSON` -> `ErrEmptyPayload`); an empty payload was never publishable on either path, so nothing that worked before stops working.
- The content-type classifier now has ONE definition, `contract.IsJSONContentType`, read by both the producer's preflight gate and the persistence codec. It was previously producer-local, and a disagreement between the two would have let an event pass preflight as opaque and then be persisted as inline JSON. One caveat worth knowing before sizing a payload: an opaque payload is base64 inside the envelope, so the 1 MiB outbox ROW cap bites about 33% earlier for one (~786 KiB of XML). That cap is the outbox column's, not the broker's; the 1 MiB Kafka record cap is unchanged and applies to the verbatim bytes.
- Export the dead-letter forensic contract so a service can DRAIN a `.dlq` topic with this library instead of opening a raw franz-go client against it. Three pieces: `streaming.DiscardHandler`, a consumer seam wired with `NewConsumer().Topics(dlqTopic).DiscardHandler(h)` that hands `HandleDiscard` each quarantine entry already decoded; `streaming.DiscardRecord` plus `streaming.ParseDiscardRecord(headers, payload)`, the typed entry and its standalone parser (origin topic/partition/offset, cause kind, error class and message, retry count, first-failure stamp, producer id, payload-omitted markers, the original CloudEvents envelope and the payload); and the eleven header keys as `streaming.DLQHeader*` with the four cause-kind values as `streaming.DLQCause*` — six of those keys are on every entry, source partition, source offset and cause kind only on a consumer quarantine, the two payload markers only when the payload was dropped. The library already provisioned the topic, quarantined into it, and stamped those headers — but the codec dropped every non-`ce-*` header before a `Handler` ran, so none of them were reachable from a consumer built with this library. No existing signature changes and no header key or value changes meaning; the origin triple the library persists as the dedup key is untouched. The writer path DOES change on one shape, described in the next entry: quarantining a record that already carries forensic headers.
- **Quarantining an entry that already carries forensic headers now REPLACES the nine hop-scoped headers instead of appending to them**, which is a change to the writer path. It only affects a record that was already quarantined once — the first quarantine of a business record emits byte-identical headers to previous versions. The nine describe a single quarantine (where it was read from, why it died, when, by whom), so appending left a re-quarantined entry carrying two of every key: nine duplicated pairs, no way to tell which coordinate belongs to which failure, and a trail back to the original record that could not be walked. It is now one clean set per entry, always describing the quarantine that produced it, and the route back is a chain followed one hop at a time — which is what makes a DLQ reader able to answer "where did this come from" on an entry that has failed more than once. The two payload markers are the exception and are deliberately carried across hops: they say the payload is not the original and how large the original was, which stays true at every later hop, so dropping them would let a re-quarantined entry claim a genuinely empty payload and lose the only surviving record of the dropped payload's size. A hop that drops the payload itself restates its own measurement rather than trusting the value it found.
- The discard seam is armed by what the caller BUILT, never by what their type implements. `DiscardHandler` is wired through an option only `ConsumerBuilder.DiscardHandler` can construct, so a business handler that happens to carry a `HandleDiscard` method and is wired with `Handler(...)` keeps both library verdicts on its ordinary topic.
- **`Build` refuses a `DiscardHandler` consumer subscribed to its own quarantine topic** (`ErrSubscribedToOwnQuarantineTopic`), and **warns — without refusing — when a plain `Handler` is**. `Source(...)` names where a consumer quarantines, so a consumer that also subscribes there republishes onto the topic it just read from and is redelivered, quarantined, redelivered — forever, reporting healthy and growing the topic without bound. Both strings are known at construction, so the hazard is named either way. Only the discard path refuses, and that path is new API in this release, so **no existing deployment can fail to build because of this change**: a plain `Handler` on that shape is a configuration earlier versions accept and that drains clean in practice (valid envelopes, an allowlisted `ce-source`, the handler returns nil), so the loop there is latent rather than active and refusing it would turn a minor upgrade into a startup outage. It now logs a WARN naming the topic and keeps running. The fix in both cases is a distinct `ce-source` for the reader — `Source("lender-dlq-desk")`, `Topics("lerian.streaming.lender.dlq")` — which costs one new Kafka topic and one new app identity in the ACL model per DLQ reader; the consumer provisions that topic itself.
- `DiscardHandler` is mutually exclusive with `Handler`, `On`/`OnFrom`, `Commands`, `UnmatchedPolicy`, `Apps` and `ExpectSources` (`ErrDiscardHandlerAndHandlerBothSet`), enforced in either order. `Apps` would subscribe the reader to fact topics, where it lifts the codec-fault quarantine and the `ce-source` check; `ExpectSources` would be validated and then ignored, because a reader never verifies `ce-source`. Previously the two whole-stream modes wrote the same field and the last call silently won — demoting a DLQ reader to a plain handler while it was still subscribed to a `.dlq` topic.
- Parsing a discard record never fails, and the origin triple fails closed as a UNIT: a present-but-unreadable coordinate discards all three and reports `ErrMalformedOriginCoordinates` on `DiscardRecord.HeaderError`, rather than yielding a plausible-looking `topic/0/42` that points at the wrong record and, as the dedup key, merges genuinely distinct quarantines. An absent coordinate is not a failure — a producer-side quarantine has no partition or offset. Envelope parse failures stay on `EnvelopeError`.
- Two library-side terminal verdicts are lifted on the `DiscardHandler` path, because a `.dlq` topic's normal content would otherwise be treated as poison: a codec fault delivers the record with a zero envelope (an unparseable envelope is exactly what a `codec` entry is, and the quarantine copy is header-verbatim), and `ce-source` verification is skipped (a quarantine copy carries the ORIGINAL producer's `ce-source`). The error the handler itself returns is deliberately NOT lifted; the refusal above is what keeps it out of a loop.
- Add `streaming.TruncatedErrorMessageBytes(msg) (int, bool)`, which reports whether a DLQ error message was CUT and how long the original was. Only a marker that ends the message counts, so an earlier hop's marker embedded in a re-quarantined entry's text does not make that message look cut. `DLQMaxErrorMessageBytes` alone does not answer it — the cut output is shorter than the bound whenever a split multi-byte rune is dropped — and detecting it otherwise means hardcoding the marker text, the restate-and-drift failure these exports exist to stop.
...
- **The outbox relay now READS version-1 envelopes, the rows lib-streaming v2 persisted, instead of destroying them.** The version gate was strict equality against `OutboxEnvelopeVersion = 2` (bumped in v3), so a version-1 row was refused as a malformed envelope. That refusal wraps `ErrInvalidOutboxEnvelope`, a CALLER error, which the lib-commons dispatcher sends straight to `INVALID` — so any service upgrading off v2 with rows still `PENDING` lost those business events at the deploy boundary, silently. `PENDING` is the normal state during a broker hiccup, which is exactly when deploys happen; the measured exposure is four production estates of br-sfn `services/slc` running `OUTBOX_ENABLED=true` with `STREAMING_ENABLED` unset, accumulating version-1 rows that would be refused the day the flag flips. Those rows are durable data a previous major wrote, and a reader that refuses them is data loss, not the absence of a compatibility layer. **Writing is unchanged — version 2 only** — and an UNKNOWN version (anything that is neither 1 nor 2) is still rejected exactly as before. (@fredcamaral)
- **A version-1 row whose destination v2 DERIVED is re-derived onto the current application topic, never published to the dead per-event name.** The original rejection guarded something real: v2 persisted a PER-EVENT destination (`midaz-ledger.transaction.created`) and v3 collapsed delivery to ONE topic per producing application (`lerian.streaming.midaz-ledger`), so replaying a v1 row verbatim would publish to a name nothing subscribes to — delivered, acknowledged, and read by nobody. The fix is not to publish it verbatim but to recompute the destination from the persisted `Event.Source` through `AppTopic`, the same helper the live Emit path uses, so the row lands exactly where a current consumer is listening. The CloudEvent, payload and partition key are untouched. Only destinations v2 DERIVED are rewritten: v2 synthesized routes from `EventDefinition.Topic(source)` but `MergeRouteOverrides` let a service aim a Kafka route anywhere, so a row whose destination is not the v2-derived name was an explicit operator choice and is left exactly as persisted — rewriting it would silently redirect a stream they still consume. Non-Kafka transports are returned unchanged at every version: the v1→v2 change was Kafka topic naming only, and an SQS queue URL or RabbitMQ exchange still addresses the same resource. Commands need no special case — the `.commands` queue postdates version 1, so every version-1 row is a business fact by construction. (@fredcamaral)
- **A version-1 row that cannot be routed is KEPT, not invalidated.** v2 fed `ce-source` through a lossy sanitizer (lowercase, punctuation-fold, separator-collapse) that v3 deleted in favour of outright rejection, so a v2-era source such as `//lerian.midaz/transaction-service` was legal to emit then and has no derivable topic now. Rather than let that become another silent `INVALID`, the relay returns the new `ErrLegacyOutboxRowUnroutable` sentinel, which is deliberately **absent from `callerErrorSentinels`** so the dispatcher takes its retryable `MarkFailed` branch. Every attempt logs at ERROR with the row id, the offending source and the reason, and increments a metric. The row is never marked `PUBLISHED`, so it is never lost. It is not immortal and the docs do not pretend otherwise: lib-commons promotes `FAILED` to `INVALID` once attempts reach `MaxDispatchAttempts` (default 10). What the retryable path buys is the whole retry budget with a loud signal on every attempt instead of one silent transition on attempt one — and an `INVALID` row is a terminal STATUS on a RETAINED row, still there to be rewritten and replayed. (@fredcamaral)
- **The two relay rejection paths are now observable**, via the new `streaming_outbox_relay_rejected_total` counter (labels `target`, `reason`) paired with an ERROR log naming the row id. It covers exactly those two reasons and no other relay failure: `version_unsupported` (undispatchable, bound for `INVALID`) and `legacy_unroutable` (kept retryable for operator action). An unregistered target keeps its existing `streaming_outbox_replay_target_unknown_total`, and other publish failures remain uncounted. This exists because the `INVALID` transition itself is effectively invisible — lib-commons performs the `FAILED → INVALID` flip inside a SQL `CASE` that emits nothing, its own "marking invalid" ERROR line is unreachable unless the service wired a retry classifier, `outbox.events.failed` counts every failure identically with no reason dimension, and the contract-package assertion on the version branch writes to a no-op logger in production. The counter and log fire independently of any `WithOnInvalid` callback. (@fredcamaral)
