...
- Add `streaming.TruncatedErrorMessageBytes(msg) (int, bool)`, which reports whether a DLQ error message was CUT and how long the original was. `DLQMaxErrorMessageBytes` alone does not answer it — the cut output is shorter than the bound whenever a split multi-byte rune is dropped — and detecting it otherwise means hardcoding the marker text, the restate-and-drift failure these exports exist to stop.
