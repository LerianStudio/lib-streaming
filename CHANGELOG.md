...
Features:
- Export the dead-letter forensic contract so a service can DRAIN its own `.dlq` topic with this library instead of opening a raw franz-go client against it. Three pieces: `streaming.DiscardHandler`, a consumer seam wired with `NewConsumer().Topics(dlqTopic).DiscardHandler(h)` that hands `HandleDiscard` each quarantine entry already decoded; `streaming.DiscardRecord` plus `streaming.ParseDiscardRecord(headers, payload)`, the typed entry and its standalone parser (origin topic/partition/offset, cause kind, error class and message, retry count, first-failure stamp, producer id, payload-omitted markers, the original CloudEvents envelope and the payload); and the eleven header keys as `streaming.DLQHeader*` with the four cause-kind values as `streaming.DLQCause*`. The library already provisioned the topic, quarantined into it, and stamped all eleven headers — but the codec dropped every non-`ce-*` header before a `Handler` ran, so none of them were reachable from a consumer built with this library. Additive: no existing signature, header value, or quarantine behavior changes, and the origin triple the library persists as the dedup key is untouched.
- Parsing a discard record NEVER fails, and two library-side terminal verdicts are lifted on the `DiscardHandler` path, because a DLQ reader's `ce-source` is the application whose `.dlq` it drains — a terminal verdict would republish onto the topic being emptied, forever. A codec fault delivers the record with a zero envelope and reports the reason in `DiscardRecord.EnvelopeError` (an unparseable envelope is exactly what a `codec` entry is, and the quarantine copy is header-verbatim), and `ce-source` verification is skipped (a quarantine copy carries the ORIGINAL producer's `ce-source`). The error the handler itself returns is deliberately NOT lifted: it is classified like any other handler error.
