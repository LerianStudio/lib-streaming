...
// newBillingSerializer wires a billing.Serializer against an in-memory schema
// registry (srfake starts its own httptest.Server) so the E2E emit path exercises
// the real Confluent-Protobuf serialization end-to-end.
func newBillingSerializer(t *testing.T) *billing.Serializer {
	t.Helper()
...
// decodeBillingRecord decodes a Confluent-framed billing record value back into a
// BillablePayload by binding a serde to the schema id carried on the wire.
func decodeBillingRecord(t *testing.T, value []byte) *billing.BillablePayload {
	t.Helper()

	require.GreaterOrEqual(t, len(value), 5, "at least magic byte + 4-byte schema id")
	require.Equal(t, byte(0x00), value[0], "Confluent magic byte")

	id := int(binary.BigEndian.Uint32(value[1:5]))

	dec := sr.NewSerde()
	dec.Register(id, &billing.BillablePayload{}, sr.Index(0),
		sr.DecodeFn(func(raw []byte, v any) error {
	...
	require.NoError(t, dec.Decode(value, &got))
...
func emitBillingEvent(t *testing.T, emitter streaming.Emitter) {
	...
	serializer := newBillingSerializer(t)

	payload, err := serializer.Serialize(&billing.BillablePayload{
		Metric:         "api_calls",
...
func TestRouteOverrides_MergesWithAutoGenerated(t *testing.T) {
	...
	// Assert the Confluent wire prefix on the RAW record value BEFORE decoding:
	// a 0x00 magic byte and a 4-byte big-endian schema id must front the bytes
	// the producer actually put on the broker.
	value := records[0].Value
	require.GreaterOrEqual(t, len(value), 5, "at least magic byte + 4-byte schema id")
	require.Equal(t, byte(0x00), value[0], "Confluent magic byte on the produced record")
	require.Positive(t, binary.BigEndian.Uint32(value[1:5]), "big-endian schema id present before proto decode")

	got := decodeBillingRecord(t, value)

	if got.GetMetric() != "api_calls" || got.GetSubscriptionId() != "sub_123" {
		t.Errorf("billing record = {Metric:%q SubscriptionId:%q}; want {api_calls sub_123}", got.GetMetric(), got.GetSubscriptionId())
	}
